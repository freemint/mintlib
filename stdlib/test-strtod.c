...
static const struct ltest tests[] = {
	...
	{ "0.1", 0.1, '\0', 0 },
	{ ".1", 0.1, '\0', 0 },
	{ "0.000000000000000000000000000000000000783475", 7.834750e-37, '\0', 0 },
	{ NULL, 0, '\0', 0 }
};
...
static int long_dbl(void)
{
	...
	double d = STRTOF(longestdbl, NULL);

	printf("strtod (\"%s\", NULL) = %g test %u", longestdbl, d, testnum);
...
int main(int argc, char **argv)
{
	...
	for (lt = tests; lt->str != NULL; ++lt)
	{
		...
		d = STRTOF(lt->str, &ep);
		save_errno = errno;
		printf("strtod (\"%s\") test %u", lt->str, testnum);
		testnum++;
		...
		} else
		{
			...
			if (save_errno != lt->err)
				printf("  errno %d (%s)  instead of %d (%s)\n",
					   save_errno, strerror(save_errno), lt->err, strerror(lt->err));
			status = 1;
		}
	}

	sprintf(buf, "%f", STRTOF("-0.0", NULL));
