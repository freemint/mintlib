...
/* Modified by Guido Flohr <guido@freemint.de>:  The expected results
   have to be canonicalized as well because it is not guaranteed that
   they are free of symlinks.  */
...
#ifdef __MINT__
  /* Our top-level directories are always soft links.  */
  else if (expected != NULL && expected[0] == '/')
    {
      char buf[PATH_MAX];
      realpath (expected, buf);
      good = (strcmp (buf, result) == 0);
    }
#endif
  else
