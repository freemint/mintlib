...
static zic_t gethms(const char *string, const char *errstring, int signable);
static zic_t getstdoff(char *field, int *isdst);
...
static int yearistype(zic_t year, const char *type);
...
static int ttisgmts[TZ_MAX_TYPES];
...
static char *ATTRIBUTE_MALLOC ecpyalloc(const char *str)
{
...
static void *growalloc(void *ptr, size_t itemsize, ptrdiff_t nitems, ptrdiff_t *nitems_alloc)
{
	...
	{
		size_t nitems_max = PTRDIFF_MAX - WORK_AROUND_QTBUG_53071;
...
static void eats(const char *name, lineno num, const char *rname, lineno rnum)
{
	filename = name;
	linenum = num;
	rfilename = rname;
...
static void eat(const char *name, lineno num)
{
...
static void ATTRIBUTE_FORMAT((printf, 1, 0)) verror(const char *const string, va_list args)
{
	...
	if (rfilename != NULL)
		fprintf(stderr, _(" (rule from \"%s\", line %" PRIdMAX ")"), rfilename, rlinenum);
	fprintf(stderr, "\n");
}
...
static void close_file(FILE *stream, const char *dir, const char *name)
{
...
static char *relname(char const *from, char const *to)
{
	...
	if (*to == '/')
	{
		...
		int needslash = len && directory[len - 1] != '/';
...
static void infile(const char *name)
{
	...
	for (num = 1;; ++num)
	{
		...
		*cp = '\0';
		...
		{
			const struct lookup *line_codes = name == leapsec ? leap_line_codes : zi_line_codes;
...
static int inzsub(char **fields, int nfields, int iscont)
{
	...
	if (max_format_len < (int) strlen(z.z_format))
		max_format_len = (int) strlen(z.z_format);
...
static void swaptypes(int i, int j)
{
	...
	{ unsigned char t = abbrinds[i]; abbrinds[i] = abbrinds[j]; abbrinds[j] = t; }
	{ int t = ttisstds[i]; ttisstds[i] = ttisstds[j]; ttisstds[j] = t; }
	{ int t = ttisgmts[i]; ttisgmts[i] = ttisgmts[j]; ttisgmts[j] = t; }
}
...
static void writezone(const char *const name, const char *const string, char version, int defaulttype)
{
	 ...
	 */
	{
		...
		{
			if (toi != 0 &&
				((attypes[fromi].at + gmtoffs[attypes[toi - 1].type]) <= (attypes[toi - 1].at + gmtoffs[toi == 1 ? 0 : attypes[toi - 2].type])))
			{
				attypes[toi - 1].type = attypes[fromi].type;
				continue;
			}
			if (toi == 0 || attypes[fromi].dontmerge || attypes[toi - 1].type != attypes[fromi].type)
	...
	for (pass = 1; pass <= 2; ++pass)
	{
		 ...
		 */
		{
			...
			{
				isdsts[mrudst] = -1;
				type = addtype(gmtoffs[mrudst], &chars[abbrinds[mrudst]], TRUE, ttisstds[mrudst], ttisgmts[mrudst]);
				isdsts[mrudst] = 1;
				omittype[type] = FALSE;
			}
			...
			{
				isdsts[mrustd] = -1;
				type = addtype(gmtoffs[mrustd], &chars[abbrinds[mrustd]], FALSE, ttisstds[mrustd], ttisgmts[mrustd]);
				isdsts[mrustd] = 0;
				omittype[type] = FALSE;
			}
		}
		...
		for (i = old0; i < typecnt; ++i)
			if (!omittype[i])
				typemap[i == old0 ? defaulttype : i == defaulttype ? old0 : i] = thistypecnt++;
...
static size_t doabbr(char *abbr, const struct zone *zp, const char *letters, int isdst, zic_t stdoff, int doquotes)
{
	register char *cp;
...
static int stringzone(char *result, const struct zone *zpfirst, ptrdiff_t zonecount)
{
	...
	if (dstrp == NULL)
		return compat;
	len += doabbr(result + len, zp, dstrp->r_abbrvar, dstrp->r_isdst, dstrp->r_stdoff, TRUE);
	if (dstrp->r_stdoff != SECSPERMIN * MINSPERHOUR)
	{
		offsetlen = stringoffset(result + len, -(zp->z_gmtoff + dstrp->r_stdoff));
		if (offsetlen == 0)
		{
			result[0] = '\0';
			return -1;
	...
	for (i = 0; i < zonecount; ++i)
	{
		...
		*startbuf = '\0';
		...
		if (zp->z_nrules == 0)
		{
			...
			type = addtype(oadd(zp->z_gmtoff, stdoff), startbuf, zp->z_isdst, startttisstd, startttisgmt);
			if (usestart)
			{
				...
				for (;;)
				{
					...
					if (usestart)
					{
						if (ktime < starttime)
						{
							...
							doabbr(startbuf, zp, rp->r_abbrvar, rp->r_isdst, rp->r_stdoff, FALSE);
							continue;
						}
					...
					eats(zp->z_filename, zp->z_linenum, rp->r_filename, rp->r_linenum);
					doabbr(ab, zp, rp->r_abbrvar, rp->r_isdst, rp->r_stdoff, FALSE);
					offset = oadd(zp->z_gmtoff, rp->r_stdoff);
					type = addtype(offset, ab, rp->r_isdst, rp->r_todisstd, rp->r_todisgmt);
		...
		{
			if (*startbuf == '\0' &&
				zp->z_format != NULL && strchr(zp->z_format, '%') == NULL && strchr(zp->z_format, '/') == NULL)
			...
			{
				int isdst = startoff != zp->z_gmtoff;
				type = addtype(startoff, startbuf, isdst, startttisstd, startttisgmt);
...
static int yearistype(zic_t year, const char *type)
{
	...
	if (type == NULL || *type == '\0')
		return TRUE;
	buf = emalloc(1 + 4 * strlen(yitcommand) + 2 + INT_STRLEN_MAXIMUM(zic_t) + 2 + 4 * strlen(type) + 2);
